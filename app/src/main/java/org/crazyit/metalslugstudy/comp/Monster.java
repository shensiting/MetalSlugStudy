...
import org.crazyit.metalslugstudy.GameView;
import org.crazyit.metalslugstudy.ViewManager;
...
public class Monster {
    ...
    //记录死亡的动画只绘制一次，不需要重复绘制
    //每当怪物死亡时，该变量会初始化为等于死亡动画的总帧数
    //当怪物死亡动画帧播放完成时，该变量的值为0
    ...
    public void drawAni(Canvas canvas, Bitmap[] bitmapArr){
        if(null==canvas||null==bitmapArr){
            return;
        }
        //如果怪物已经死亡，且没有播放过死亡动画，dieMaxDrawCount等于初始值为未播放过死亡动画
    ...
    public int getBulletType(){
        switch (type){
            case TYPE_BOMB:return 0;
            case TYPE_FLY:return Bullet.BULLET_TYPE_3;
            case TYPE_MAN:return Bullet.BULLET_TYPE_2;
            default:return 0;
        }
    }
    ...
    public void drawBullet(Canvas canvas){
        ...
        for(int i=0;i<bulletList.size();i++){
            ...
            // 如果子弹已经越过屏幕
            if(bullet.getX()<0||bullet.getY()>ViewManager.SCREEN_WIDTH){
    ...
    public void checkBullet(){
        ...
        //遍历所有子弹
        for(Bullet bullet:bulletList){
            if(null==bullet||!bullet.isEffect()){
              continue;
            }  // 如果玩家控制的角色被子弹打到
