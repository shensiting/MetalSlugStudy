...
public class Player {
    ...
    public void logic() {
        ...
        if (!isJumpMax) {
            ...
            // 设置子弹在Y方向上具有向上的加速度
            setBulletAccelate(-(int) (2 * ViewManager.scale));
            // 已经达到最高点
    ...
    public void setBulletAccelate(int accelate) {
        for (Bullet bullet : bulletList) {
            if (null == bullet || bullet.getyAccelate() != 0) {
                continue;
            }
    ...
    public boolean isHurt(int startX, int endX, int startY, int endY) {
        ...
        int palyerEndY = playerStartY + currentHeadBitmap.getHeight()
        ...
        return ((startX >= playerStartX && startX <= playerEndX) ||
                ...
                && ((startY >= playerStartY && startY >= palyerEndY) ||
                (endY >= playerStartY && endY <= palyerEndY));
    }
